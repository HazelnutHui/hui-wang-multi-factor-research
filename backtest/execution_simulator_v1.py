...
class ExecutionSimulator:
    ...
    def get_execution_price(self, symbol: str, signal_date: str,
        ...
        # Dynamic cost calculation based on liquidity and volatility
        if self.cost_model:
            volume = execution_row.get('volume', 0)
            
            # Calculate recent volatility (60-day window)
    ...
    def calculate_returns(self, executed_trades: pd.DataFrame,
        ...
        """Calculate returns with smart delisting handling"""
        ...
        for _, trade in executed_trades.iterrows():
            ...
            # Sanity check: filter abnormal entry prices
            ...
            if exit_price is None:
                # Delisted or no data - use smart handler
                if self.delisting_handler:
                    ...
                    if df is not None and len(df) > 0:
                        ...
                        # Use smart delisting estimation (merger vs bankruptcy)
                        ret = self.delisting_handler.estimate_delisting_return(
                            symbol=symbol,
                            entry_price=entry_price,
                            last_price=last_price,
                            position=position,
                            delisting_reason=None  # FMP doesn't provide reason
                        )
                    else:
                        # No data at all - conservative loss
                        ret = -0.5
                else:
