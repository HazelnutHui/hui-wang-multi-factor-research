...
_spec = _ilu.spec_from_file_location("value_v2_config", _cfg_path)
...
def _resolve_path(p: str) -> str:
    ...
    # Prefer repo root for data/* paths; otherwise resolve relative to backtest/
...
def _make_engine_config():
    ...
    return {
        ...
        'VALUE_MAINSTREAM_COMPOSITE': getattr(cfg, 'VALUE_MAINSTREAM_COMPOSITE', False),
        'VALUE_COMPONENT_TRANSFORM': getattr(cfg, 'VALUE_COMPONENT_TRANSFORM', 'signed_log'),
        'VALUE_COMPONENT_INDUSTRY_ZSCORE': getattr(cfg, 'VALUE_COMPONENT_INDUSTRY_ZSCORE', False),
        'VALUE_COMPONENT_WINSOR_PCT_LOW': getattr(cfg, 'VALUE_COMPONENT_WINSOR_PCT_LOW', None),
        'VALUE_COMPONENT_WINSOR_PCT_HIGH': getattr(cfg, 'VALUE_COMPONENT_WINSOR_PCT_HIGH', None),
        'VALUE_COMPONENT_MIN_COUNT': getattr(cfg, 'VALUE_COMPONENT_MIN_COUNT', 1),
        'VALUE_COMPONENT_MISSING_POLICY': getattr(cfg, 'VALUE_COMPONENT_MISSING_POLICY', 'drop'),
    }
...
def main():
    ...
    parser.add_argument("--exec-trading-days", action="store_true",
                        help="Use trading-day execution (does not change config defaults).")
    parser.add_argument("--trade-size-usd", type=float, default=None,
                        help="Optional trade size for cost diagnostics (USD).")
    args = parser.parse_args()

    cfg_dict = _make_engine_config()
    if args.exec_trading_days:
        cfg_dict["EXECUTION_USE_TRADING_DAYS"] = True
    if args.trade_size_usd is not None:
        cfg_dict["TRADE_SIZE_USD"] = float(args.trade_size_usd)
    engine = BacktestEngine(cfg_dict)

    factor_weights = {'momentum': 0.0, 'reversal': 0.0, 'low_vol': 0.0, 'pead': 0.0, 'quality': 0.0, 'value': 1.0}
    ...
    report = {
        ...
        "config": {
            ...
            "rebalance_freq": cfg.REBALANCE_FREQ,
            "execution_delay": cfg.EXECUTION_DELAY,
            "execution_use_trading_days": bool(cfg_dict.get("EXECUTION_USE_TRADING_DAYS", False)),
            "enable_dynamic_cost": bool(cfg_dict.get("ENABLE_DYNAMIC_COST", False)),
            "trade_size_usd": float(cfg_dict.get("TRADE_SIZE_USD", 10000)),
            "transaction_cost": cfg.TRANSACTION_COST,
            "use_adj_prices": getattr(cfg, "USE_ADJ_PRICES", False),
        },
        "performance": {
            "train": results['train']['analysis'],
            "test": results['test']['analysis'],
            "oos": results.get('oos_analysis', {}),
            "train_n_signals": int(len(results['train']['signals'])),
            "test_n_signals": int(len(results['test']['signals'])),
            "train_n_dates": int(results['train']['signals']['date'].nunique()) if len(results['train']['signals']) else 0,
            "test_n_dates": int(results['test']['signals']['date'].nunique()) if len(results['test']['signals']) else 0,
        },
        "output_files": {
            "csv_files": {
    ...
    print(f"Signals (test):     {len(results['test']['signals'])}")
