...
_spec = _ilu.spec_from_file_location("quality_v2_config", _cfg_path)
...
def _make_engine_config():
    ...
    return {
        ...
        'QUALITY_MAINSTREAM_COMPOSITE': getattr(cfg, 'QUALITY_MAINSTREAM_COMPOSITE', False),
        'QUALITY_COMPONENT_TRANSFORM': getattr(cfg, 'QUALITY_COMPONENT_TRANSFORM', 'signed_log'),
        'QUALITY_COMPONENT_INDUSTRY_ZSCORE': getattr(cfg, 'QUALITY_COMPONENT_INDUSTRY_ZSCORE', False),
        'QUALITY_COMPONENT_WINSOR_PCT_LOW': getattr(cfg, 'QUALITY_COMPONENT_WINSOR_PCT_LOW', None),
        'QUALITY_COMPONENT_WINSOR_PCT_HIGH': getattr(cfg, 'QUALITY_COMPONENT_WINSOR_PCT_HIGH', None),
        'QUALITY_COMPONENT_MIN_COUNT': getattr(cfg, 'QUALITY_COMPONENT_MIN_COUNT', 1),
        'QUALITY_COMPONENT_MISSING_POLICY': getattr(cfg, 'QUALITY_COMPONENT_MISSING_POLICY', 'drop'),
    }
...
def main():
    ...
    factor_weights = {'momentum': 0.0, 'reversal': 0.0, 'low_vol': 0.0, 'pead': 0.0, 'quality': 1.0}
